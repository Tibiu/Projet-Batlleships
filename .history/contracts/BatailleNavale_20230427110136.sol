...
contract BatailleNavale {
    ...
    function placerBateau(uint8 x, uint8 y) public {
        require(grille[x][y].x == 0 && grille[x][y].y == 0, "Un bateau est déjà placé sur cette case");
    ...
    function attaquer(uint8 x, uint8 y) public {
        ...
        require(grille[x][y].x != 0 || grille[x][y].y != 0, "Aucun bateau n'est présent sur cette case");
        require(!grille[x][y].coule, "Ce bateau est déjà coulé");
        bool reussite = false;
